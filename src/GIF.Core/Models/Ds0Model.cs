...
namespace GIF.Core.Models
{
    ...
    public class Ds0AdmModel
    {
        [Name("AddressID"), Index(1)]
        public string? AdmAddressID { get; set; }
        ...
        public Ds0AdmModel Clone()
        {
            var serialized = JsonConvert.SerializeObject(this);
            return JsonConvert.DeserializeObject<Ds0AdmModel>(serialized) ?? throw new InvalidCastException("Cannot clone object.");
        }
    }
    ...
    public class Ds0AdmRequest
    {
        ...
        public string CoCode { get; set; } = default!;
        [Required]
        public bool GenSqlMock { get; set; } = false;

        public string GetIdentity()
            => $"{PostCode}-{HouseNumber}-{HouseExtension}-{RoomEndNumber}".ToUpper();
    }

    public class Ds0AdmResponse
    {
        public IEnumerable<Ds0AdmModel> Ds0AdmModels { get; set; } = [];
